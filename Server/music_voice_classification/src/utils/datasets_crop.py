...
for g in genres:
    ...
    for filename in os.listdir(os.path.join('../../content/train_datasets/audio_compress', f"{g}")):                 # 여기에 기존 장르별 본래 노래(wav) 저장
        song = os.path.join(f'../../content/train_datasets/audio_compress/{g}', f'{filename}')
        j = j + 1
