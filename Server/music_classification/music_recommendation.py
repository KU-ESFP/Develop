...
def saveOutputSong(song_info, emotion):
    ...
    f = open(os.path.join(root_dir + output_path, emotion + '.txt'), 'a', encoding='UTF8')
    f.write(song_info + '\n')
    f.close()
...
def deleteOutputSong():
    output_path = root_dir + "/output_top100/"
...
path = root_dir + '/trained_models/txt_classification_02.pth'
...
'''
# path name
top100_path = 'music_classification/input_top100'
# top100_path = 'input_test'
save_path = './csv_datasets/'
input_dataset = 'music_classification/input_test_data.csv'
...
'''
crawling_path = '/src/crawlings/input_crawling.py'
exec(open(root_dir + crawling_path, encoding='utf8').read())
